...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    // 加载高级配置
    {
        ...
        if (nvsWifi.NvsNamespaceOpen()) {
            ...
            ota_url = nvsWifi.GetStr(std::string("ota_url"), &err);
            if (err == ESP_OK) {
                ota_url_ = ota_url.c_str();
            }

            // 读取WiFi功率
            max_tx_power_ = nvsWifi.GetI8(std::string("max_tx_power"), &err);
            if (err == ESP_OK) {
            ...
            remember_bssid = nvsWifi.GetU8(std::string("remember_bssid"), &err);
            if (err == ESP_OK) {
            ...
            sleep_mode = nvsWifi.GetU8(std::string("sleep_mode"), &err);
            if (err == ESP_OK) {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
...

            {
                ...
                if (cJSON_IsString(ota_url) && ota_url->valuestring) {
                    ...
                    err = nvsWifi.SetStr(std::string("ota_url"), this_->ota_url_);
                    if (err != ESP_OK) {
                ...
                if (cJSON_IsNumber(max_tx_power)) {
                    ...
                    err = nvsWifi.SetI8(std::string("max_tx_power"), this_->max_tx_power_);
                    if (err != ESP_OK) {
                ...
                if (cJSON_IsBool(remember_bssid)) {
                    ...
                    err = nvsWifi.SetU8(std::string("remember_bssid"), this_->remember_bssid_ ? 1 : 0);
                    if (err != ESP_OK) {
                ...
                if (cJSON_IsBool(sleep_mode)) {
                    ...
                    err = nvsWifi.SetU8(std::string("sleep_mode"), this_->sleep_mode_ ? 1 : 0);
                    if (err != ESP_OK) {
